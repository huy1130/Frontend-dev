            ...
            <div
              ...
              className={`rounded-3xl p-8 border flex flex-col justify-between relative transition-all duration-300 ${
                combo.isBestSeller
                  ...
                  : 'bg-white dark:bg-dark-800/80 border-slate-200 dark:border-white/10 shadow-md dark:shadow-none hover:shadow-xl'
              }`}
            >
              {/* Badges */}
              <div className="flex items-center justify-between gap-2 mb-6">
                {combo.isBestSeller ? (
                  ...
                  <span className="bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-slate-300 font-semibold text-xs px-3 py-1 rounded-full border border-slate-200 dark:border-transparent">
                    Gói Tiêu Chuẩn
                  </span>
                )}

                {combo.saveBadge && (
                  <span className="bg-amber-500/10 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/40 font-bold text-xs px-3 py-1 rounded-full">
                    {combo.saveBadge}
                  </span>
                )}
              </div>
              ...
              <div className="space-y-3 mb-6">
                <h3 className="text-2xl font-black text-slate-900 dark:text-white">{combo.name}</h3>
                <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">{combo.tagline}</p>
              </div>

              {/* Pricing Block */}
              <div className="py-5 border-y border-slate-200 dark:border-white/10 mb-6 space-y-1">
                <div className="flex items-baseline gap-3">
                  <span className="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white">
                    {combo.discountedPrice.toLocaleString()}đ
                  </span>
                  <span className="text-base text-slate-400 dark:text-slate-500 line-through font-medium">
                ...
                <ul className="space-y-2.5">
                  {combo.servicesIncluded.map((serviceStr, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
                      <div className="w-4 h-4 rounded-full bg-brand-500/10 dark:bg-brand-500/20 border border-brand-500/40 flex items-center justify-center shrink-0 mt-0.5">
              ...
              <a
                ...
                className={`w-full py-4 rounded-2xl font-bold text-center transition-all flex items-center justify-center gap-2 group ${
                  combo.isBestSeller
                    ...
                    : 'bg-slate-100 dark:bg-white/10 hover:bg-brand-600 hover:text-white dark:hover:bg-white/20 text-slate-900 dark:text-white border border-slate-200 dark:border-white/10'
                }`}
              >
                Đặt Gói Này Ngay

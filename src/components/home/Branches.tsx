...
import { MapPin, Phone, Star, Clock, CheckCircle2, Navigation } from 'lucide-react'
import { mockBranches } from '../../mock/homeData'

export default function Branches() {
  return (
    <section id="branches" className="py-24 bg-dark-800/40 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {mockBranches.map((br) => (
            <div
              key={br.id}
              className="glass-panel rounded-3xl overflow-hidden border border-white/10 glass-panel-hover flex flex-col justify-between group"
            >
              <div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="space-y-2 text-xs text-slate-300">
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="font-semibold text-white">{br.phone}</span>
                    </div>
                  ...
                  <div className="pt-3 border-t border-white/10 space-y-2">
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Tiện ích cơ sở:</p>
                    <div className="flex flex-wrap gap-1.5">
                      {br.facilities.map((fac, idx) => (
                        <span
                          key={idx}
                          className="text-[11px] px-2.5 py-1 rounded-md bg-white/5 text-slate-300 border border-white/5"
                        >
                          {fac}
                        </span>
                      ))}
              ...
              <div className="p-6 pt-0">
                <a
                  href="#booking"
                  className="w-full py-3 rounded-xl bg-brand-500/20 hover:bg-brand-500 text-brand-300 hover:text-white font-bold text-xs transition-all flex items-center justify-center gap-2 border border-brand-500/30"
                >
